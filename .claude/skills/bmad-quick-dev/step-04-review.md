...
After classification is complete and all patch fixes are applied, invoke the orchestrate skill at `@.claude/skills/orchestrate/` in REVIEW mode on all changed files. This delegates to CashOut domain guardians (ios-swiftui-guardian, cloudkit-sync-guardian, architecture-guardian) in parallel.

- Present the orchestrator report to the user alongside the review findings.
- CRITICAL findings from the orchestrator must be treated as `patch` items and auto-fixed before proceeding.
- WARNING findings from the orchestrator should be included in the final report as recommendations.
