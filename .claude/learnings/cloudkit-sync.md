...
- **2026-04-08**: Crash-prevention guards for Core Data store availability (e.g., preventing `NSInternalInconsistencyException` on `context.save()` with zero stores) belong inside the repository method itself (`coordinator.persistentStores.isEmpty`), not at the call site. Caller-side guards (`storeLoadError == nil`, `privatePersistentStore != nil`) are fragile — they can be wrong about which store is needed and silently block operations that would succeed.
