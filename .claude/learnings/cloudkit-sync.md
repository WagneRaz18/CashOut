...
- **2026-04-08**: Repository post-save sharing must be a separate method (`shareNewExpenseToHousehold(id:)`) called by the ViewModel AFTER signaling UI success (`saveCount += 1`). A `Task { @MainActor in }` inside `saveExpense` enqueues the sharing job on the main actor before `onChange(of: saveCount)` fires — `container.share()` does synchronous prep work that blocks the main run loop, delaying the success animation. The ViewModel stores the Task handle (`shareTask`) and cancels in cleanup.
