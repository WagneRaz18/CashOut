...
- **2026-04-08**: `storeLoadError` is set when ANY store fails (private OR shared). Guards that protect private-store-only operations (e.g., category seeding) must check `privatePersistentStore != nil` instead — a shared-store failure should not block seeding into the working private store. The overly broad `storeLoadError == nil` guard silently disabled the entire entry flow.
