...
- **2026-04-09**: Repository post-save sharing must be a separate fire-and-forget `Task` in the ViewModel — `container.share()` does synchronous prep work that blocks the main run loop. The ViewModel stores the Task handle (`shareTask`) and cancels in cleanup. Re-fetching by UUID inside `shareNewExpenseToHousehold` is acceptable — Core Data's identity map returns the in-memory object without disk I/O.
