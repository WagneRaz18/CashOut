...
- **2026-04-06**: Never use `max(fixedMin, calculatedHeight)` inside GeometryReader to size grid children — if `fixedMin × rowCount` exceeds the GeometryReader's frame, content overflows visually (GeometryReader does NOT clip) and overlaps siblings below. Use `max(0, calculatedHeight)` to prevent negative values while letting keys scale down to fit.
