...
- **W3: `@unchecked Sendable` on PersistenceController** — ~~`PersistenceController.swift:4`~~. **RESOLVED 2026-04-10**: class is now `@MainActor final class`. All callers were already main-actor; `AppDelegate` was annotated `@MainActor` in the same pass.
- **W4: `handleAccountChange` observer is a no-op** — ~~`PersistenceController.swift:94-97`~~. **RESOLVED 2026-04-10**: replaced by `observeAccountChanges()` async sequence (`PersistenceController.swift:258-272`) invoked from a long-lived `.task {}` on `CashOutApp`'s root `Group`. On change, nils both store references and `storeLoadError`, then posts `accountDidChange`. **Residual concern — ordering non-guarantee**: there are now four listeners for iCloud account changes (three on raw `.CKAccountChanged` — `PersistenceController.observeAccountChanges()`, `SyncMonitorService.accountTask`, `AuthenticationService.accountChangeTask` — plus one derived listener on `PersistenceController.accountDidChange` in `ContentView`). The raw listeners fire concurrently with no ordering contract, so keychain-clear vs. store-teardown vs. sharing-status-recheck may interleave. Acceptable at 2-user personal scale; revisit if multi-device or multi-account support is ever added.
- **W5: `purgeOldHistory` runs synchronously on main thread** — ~~`PersistenceController.swift:73`~~. **RESOLVED 2026-04-10**: `purgeOldHistory()` is now `async`, uses `await context.perform { }`, scopes to `affectedStores = [privateStore]`, and is invoked via `async let` from `CashOutApp`'s startup `.task {}` concurrent with splash/seeding/auth.
...
## Deferred from: code review of 5-2-custom-category-creation-and-editing (2026-04-06)

- **D1: prepareObjectForSharedSave + shareObjectsToHouseholdIfNeeded called unconditionally** — `CategoryRepository.swift:52-60`. Both pre-save (participant path) and post-save (owner path) are called for all new custom categories without checking `isShareOwner`. Pre-existing pattern matching ExpenseRepository.
- **D2: AC #7 edit sync: updates to existing categories skip shared-zone routing** — `CategoryRepository.swift:41,52`. `isNewCustomCategory` check skips both sharing calls for edits. NSPersistentCloudKitContainer auto-syncs dirty records already in the shared store, so edits DO sync for categories that were successfully shared on creation. Gap only affects categories where initial share silently failed.
- **D3: @ObservationIgnored on let constants is redundant** — `SettingsViewModel.swift:21-28`. All four dependencies are `let` constants; `@Observable` never tracks them. Pre-existing across all ViewModels in the project.
- **D4: Unstructured Task {} in HouseholdSectionView invitePartner** — `SettingsView.swift:87`. Fire-and-forget task with no stored handle or .onDisappear cancellation. Pre-existing from Story 4-1.

## Deferred from: insights-monthly-calendar-view (2026-05-02)

- **D1: MonthlyCalendarView today-marker staleness at midnight** — `MonthlyCalendarView.swift:12-14`. `todayStart` is derived from `today: Date()` passed at `InsightsView.body` evaluation. No state change triggers re-evaluation at midnight, so the today circle can stay on the wrong day until next render. Cosmetic only; fix requires a timer (`.onReceive(Timer.publish(every: 60, on: .main, in: .common))`).
- **D2: MonthlyCalendarView partial last row** — `MonthlyCalendarView.swift:buildCells()`. Months ending mid-week produce a partial last row (e.g. 2 cells on the last row of a 6-row month). Visual inconsistency only. Fix: append `((7 - cells.count % 7) % 7)` trailing nil filler cells after the day loop.
- **D3: navigateToDay(today) may skip reload when loadedPeriod/offset cached as .daily/0** — `InsightsViewModel.swift:215-218`. If the user was previously on the daily "today" view, returning via calendar tap hits the existing `loadedPeriod == .daily && loadedOffset == 0` dedup guard and skips reload. Remote sync data delivered while on monthly view would not appear. Pre-existing load-guard pattern; fix: invalidate `loadedPeriod` in `navigateToDay` before mutating state.
- **D4: periodBinding setter fires two @Observable mutations in one call** — `InsightsView.swift:14-18`. Sets `selectedPeriod` then calls `resetToCurrentPeriod()` (sets `dateOffset = 0`). Two mutations may produce two render passes vs. the prior `.onChange` which was post-render. Functional behavior is correct; refactor to `func switchPeriod(_ period: TimePeriod)` on ViewModel for single-pass batching.

## Deferred from: spec-insights-category-filter (2026-05-03)

- **D1: Category filter proportion accessibility label** — `CategoryBreakdownView.swift:45`. When filter active, the accessibility label says "X% of total" but percentage is computed against `filteredTotalAmount` (visible-only denominator), not the true period total. Cosmetic/semantic only; proportion bars and headline are consistent with each other.
- **D2: 2pt minimum bar on excluded rows** — `CategoryBreakdownView.swift:37`. `max(geometry.size.width * proportion, 2)` is a pre-existing floor that renders a hairline on excluded rows even though proportion is forced to 0.0. Cosmetic only; 0.35 opacity dimming already communicates exclusion.
- **D3: Double chart angle selection** — `InsightsSummaryView.swift:57-63`. `.chartGesture` SpatialTapGesture and `.chartAngleSelection` can both fire for one physical tap in certain cadences, calling `onSliceTapped` twice. Pre-existing; no user-visible impact since second call sets `selectedDestination` to same value.
